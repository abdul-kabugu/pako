import {
  ...
  export default function BottomNav() {
    ...
    return (
      <div className='fixed bg-black  bottom-0 left-0 right-0 flex h-12 w-full items-center  justify-evenly gap-3 border-t border-gray-500  pt-1 dark:border-border-gray  md:hidden'>
        <Link href={'/'}>
          <div>
            <HomeOutline className='h-5 w-5 text-white z-40' />
          </div>
        </Link>

        <Link href={'/upload'}>
          <div>
          <CiCirclePlus className='h-7 w-7 cursor-pointer text-white' />  
                  </div>
        </Link>
        <Link href={'/shorts'}>
          <div>
            <ShortsOutline className='h-5 w-5 text-white' />
          </div>
        </Link>
