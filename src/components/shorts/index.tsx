...
const user = useUser()
  ...
  console.log("the current user  oooo  oooo", user)
 ...
 const [profileId, postId] = id ? id.split("-") : [null, null];
  ...
  const [sliderRef, { current: slider }, instanceRef] = useKeenSlider(
    {
      vertical: true,
...
console.log("the single byte", singleByte)
  ...
  }, [allFilterePosts]); // Run effect only once on component mount
  ...
  return (
    <div className='relative h-[calc(100vh-0rem)] overflow-y-hidden  focus-visible:outline-none md:h-[calc(100vh-4rem)]'>
      <MetaTags title="Shorts" />
      <div
        ref={sliderRef}
        className='keen-slider h-[calc(100vh-3rem)]  snap-y snap-mandatory focus-visible:outline-none md:h-[calc(100vh-4rem)]'
      >
        {singleByte && (
          <ShortVideo
            video={singleByte}
     ...
     {/*} .filter(video => video?.noteId !==  postId)*/}
      ...
      <div className='laptop:right-6 ultrawide:right-8 bottom-2 right-4 hidden flex-col space-y-2 md:absolute md:flex'>
        <button
          className='bg-gallery rounded-full p-3 focus:outline-none dark:bg-gray-800 bg-sky-100'
          onClick={() => slider?.prev()}
        >
          <svg
